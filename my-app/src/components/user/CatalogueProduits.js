...
import '../../Styles/Catalogueproduits.css';
...
const Catalogue = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      const url = isLoggedIn
        ...
        ? 'http://localhost:5000/api/products/allwithrecommended'
      ...
      try {
        ...
        setProduits(response.data.data || response.data); // Gérer les deux structures de réponse

        const ids = (response.data.data || response.data).map(p => p.id);
  ...
  }, [isLoggedIn]); // Ré-exécuter l'effet lorsque l'état de connexion change
          ...
          {currentProduits.map(produit => (
            ...
            <div key={produit.id} className="produit-card">
              ...
              <div className="product-info">
                ...
                {produit.promo_price !== null && produit.promo_price !== undefined ? (

                  <div className="price-container">

                    <span className="old-price">{produit.promo_price} TND</span>

                    <span className="new-price">{produit.price} TND</span>
